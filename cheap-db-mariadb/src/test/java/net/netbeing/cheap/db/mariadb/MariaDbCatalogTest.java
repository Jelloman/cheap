...
class MariaDbCatalogTest
{
    abstract static class MariaDbCatalogTestBase
    {
            ...
            try (Connection connection = getDb().dataSource.getConnection();
                ...
                statement.execute(
                    "INSERT INTO test_table " +
                        "(id, string_col, integer_col, float_col, date_col, timestamp_col, boolean_col, uuid_col, " +
                        "blob_col) " +
        ...
        void testLoadTableDef()
        {
            ...
            List<? extends PropertyDef> properties = tableDef.propertyDefs()
                .stream().toList();
        ...
        void testLoadTable()
        {
            ...
            for (Aspect aspect : table.values()) {
                ...
                if (idValue == 1) {
                    ...
                    assertEquals("2025-01-01T00:00Z[UTC]", dateProp.read()
                        .toString(), "date_col should be 2025-01-01T00:00Z[GMT]");

                    Property timestampProp = aspect.get("timestamp_col");
                    assertEquals("2025-01-11T18:18:18Z[UTC]", timestampProp.read()
                        .toString(), "timestamp_col should be 2025-01-11T18:18:18Z[GMT]");
                    ...
                    assertEquals("4186bfb6-b135-48af-9236-95cacdb20327", uuidProp.read(), "uuid_col should match " +
                        "expected UUID");
                    ...
                    Property dateProp = aspect.get("date_col");
                    assertEquals("2025-02-02T00:00Z[UTC]", dateProp.read()
                        .toString(), "date_col should be 2025-02-02T00:00Z[GMT]");

                    Property timestampProp = aspect.get("timestamp_col");
                    assertEquals("2025-02-02T02:02:02Z[UTC]", timestampProp.read()
                        .toString(), "timestamp_col should be 2025-02-02T02:02:02Z[GMT]");

                    Property booleanProp = aspect.get("boolean_col");
                    assertEquals(false, booleanProp.read(), "boolean_col should be false");

                    Property uuidProp = aspect.get("uuid_col");
                    assertEquals("655a99b9-af7c-4f2f-afa8-c4801986b9d4", uuidProp.read(), "uuid_col should match " +
                        "expected UUID");
                }
            }

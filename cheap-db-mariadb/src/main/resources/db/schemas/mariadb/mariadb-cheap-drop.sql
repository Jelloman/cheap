-- MariaDB Drop DDL for Cheap Data Model
-- Drops all tables, functions, and other database objects created by mariadb-cheap.sql and mariadb-cheap-audit.sql
-- This script should be executed to completely clean up the Cheap schema

-- ========== DROP TRIGGERS ==========

-- Drop audit triggers (from mariadb-cheap-audit.sql)
...
-- Drop hierarchy content tables (from mariadb-cheap.sql)
...
-- Drop property value table (from mariadb-cheap.sql)
DROP TABLE IF EXISTS property_value;

-- ========== DROP CORE ENTITY TABLES ==========

-- Drop core entity tables (from mariadb-cheap.sql)
...
-- Drop definition tables (from mariadb-cheap.sql)

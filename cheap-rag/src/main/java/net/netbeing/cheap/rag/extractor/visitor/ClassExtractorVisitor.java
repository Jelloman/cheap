...
/**
 * Visitor for extracting class declarations from Java code.
 */
public class ClassExtractorVisitor extends VoidVisitorAdapter<List<MetadataArtifact>>
{
    ...
    public ClassExtractorVisitor(String sourceFile, JavaExtractorConfig config)
    {
    ...
    {
        // Skip interfaces - they're handled by InterfaceExtractorVisitor
        ...
        String qualifiedName = declaration.getFullyQualifiedName().orElse(className);
        String packageName = getPackageName(qualifiedName, className);
        ...
        declaration.getImplementedTypes().forEach(type -> implementsTypes.add(type.getNameAsString()));

        if (!extendsTypes.isEmpty()) {
            embeddingText.append("\nExtends: ").append(String.join(", ", extendsTypes));
        }
        if (!implementsTypes.isEmpty()) {
            embeddingText.append("\nImplements: ").append(String.join(", ", implementsTypes));
        }

        // Generate ID
        String id = IdGenerator.generateId("java", ArtifactType.CLASS, qualifiedName);
    ...
    private String getPackageName(String qualifiedName, String className)
    {
